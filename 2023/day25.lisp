...
(defun parse (input)
  (let ((connections (mapcar (lambda (line) (read-from-string (format nil "(~a)" (aoc:tr ":" " " line))))
    ...
    (let ((edges (remove-duplicates (a:flatten (mapcar #'edges (a:hash-table-values nodes))))))
      (make-array (length edges) :initial-contents edges :fill-pointer t))))
...
(defun contract (edge)
  (let ((a (a edge))
        (b (b edge)))
    (mapc (a:curry #'replace-node a b) (edges b))
    (setf (edges a) (delete edge (edges a)))
    a))

(defun replace-node (new old edge)
  (when (eq old (a edge)) (setf (a edge) new))
  (when (eq old (b edge)) (setf (b edge) new)))
